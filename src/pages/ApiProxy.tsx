...
export default function ApiProxy() {
    ...
    // Copilot OAuth states
    const [copilotOAuthLoading, setCopilotOAuthLoading] = useState(false);
    const [copilotDeviceCode, setCopilotDeviceCode] = useState<{ user_code: string; verification_uri: string; device_code: string; interval: number } | null>(null);
    const [copilotOAuthMessage, setCopilotOAuthMessage] = useState<{ type: 'success' | 'error'; text: string } | null>(null);
    ...
    // ⚠️ Copilot OAuth 登录处理
    const handleCopilotOAuth = async () => {
        setCopilotOAuthLoading(true);
        setCopilotOAuthMessage(null);
        setCopilotDeviceCode(null);

        try {
            // 步骤 1: 获取设备码
            const deviceCode = await invoke<{
                user_code: string;
            ...
            // 步骤 2: 轮询等待用户授权
            const oauthToken = await invoke<string>('poll_copilot_oauth_token', {
                deviceCode: deviceCode.device_code,
                interval: deviceCode.interval
            });

            // 步骤 3: 使用 OAuth token 获取 Copilot token
            const copilotToken = await invoke<string>('get_copilot_token_from_oauth', {
                oauthToken
            });

            // 保存获取到的 Copilot token
            updateCopilotConfig({ github_token: copilotToken });

            setCopilotOAuthMessage({ type: 'success', text: 'Copilot token 获取成功！' });
            setCopilotDeviceCode(null);
            showToast('Copilot 登录成功', 'success');
        } catch (error: any) {
            console.error('Copilot OAuth error:', error);
            setCopilotOAuthMessage({ type: 'error', text: error.toString() || '登录失败，请重试' });
        } finally {
            setCopilotOAuthLoading(false);
        }
    };
                ...
                {/* External Providers Integration */}
                {
                    appConfig && (
                        <div className="space-y-4">
                            ...
                            {/* ⚠️ Copilot Provider - 非官方 API，使用风险自负 */}
                            <CollapsibleCard
                                title={t('proxy.config.copilot.title')}
                                icon={<Github size={18} className="text-gray-700 dark:text-gray-300" />}
                                enabled={!!appConfig.proxy.copilot?.enabled}
                                onToggle={(checked) => updateCopilotConfig({ enabled: checked })}
                                rightElement={
                                    <div className="flex items-center gap-1 text-[10px] text-amber-500 dark:text-amber-400">
                                        <AlertTriangle size={12} />
                                        <span className="hidden md:inline">非官方</span>
                                    </div>
                                }
                            >
                                <div className="space-y-4">
                                    ...
                                    <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-lg p-3">
                                        <div className="flex items-start gap-2 text-amber-800 dark:text-amber-200">
                                            ...
                                            <div className="text-[11px] space-y-1">
                                                <div className="font-bold">⚠️ {t('proxy.config.copilot.warning.title')}</div>
                                                <div className="opacity-80">{t('proxy.config.copilot.warning.risk')}</div>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        <div className="space-y-1">
                                            <label className="text-[11px] font-medium text-gray-500 dark:text-gray-400 flex items-center justify-between">
                                                <span>{t('proxy.config.copilot.github_token')}</span>
                                                {!(appConfig.proxy.copilot?.github_token) && (
                                                    <span className="text-amber-500 text-[10px] flex items-center gap-1">
                                                        {t('common.required')}
                                                    </span>
                                                )}
                                            ...
                                            <div className="flex gap-2">
                                                <input
                                                    type="password"
                                                    value={appConfig.proxy.copilot?.github_token || ''}
                                                    onChange={(e) => updateCopilotConfig({ github_token: e.target.value })}
                                                    placeholder="ghp_xxx 或 gho_xxx"
                                                    className="input input-sm input-bordered flex-1 font-mono text-xs"
                                                />
                                                <button
                                                    onClick={handleCopilotOAuth}
                                                    disabled={copilotOAuthLoading}
                                                    className="btn btn-sm btn-outline"
                                                    title="通过 GitHub OAuth 登录获取 token"
                                                >
                                                    {copilotOAuthLoading ? (
                                            ...
                                            <div className="text-[10px] text-gray-400">
                                                {t('proxy.config.copilot.github_token_hint')}
                                            </div>
                                            {copilotOAuthMessage && (
                                                <div className={`text-[10px] p-2 rounded ${copilotOAuthMessage.type === 'error' ? 'bg-red-50 text-red-600 dark:bg-red-900/20 dark:text-red-400' : 'bg-green-50 text-green-600 dark:bg-green-900/20 dark:text-green-400'}`}>
                                                    {copilotOAuthMessage.text}
                                                </div>
                                            )}
                                            {copilotDeviceCode && (
                                                <div className="p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
                                                    <div className="text-sm font-medium mb-2">请在浏览器中完成授权:</div>
                                                    <div className="text-[10px] text-gray-600 dark:text-gray-400 mb-2">
                                                        1. 点击下方链接打开 GitHub<br/>
                                                        2. 输入设备码并确认
                                                    </div>
                                                    <div className="flex items-center gap-2 mb-2">
                                                        <input
                                                            readOnly
                                                            value={copilotDeviceCode.verification_uri}
                                                            className="input input-xs input-bordered flex-1 font-mono"
                                                        />
                                                        <button
                                                            onClick={() => window.open(copilotDeviceCode.verification_uri, '_blank')}
                                                            className="btn btn-xs btn-primary"
                                                        >
                                                            打开链接
                                                        </button>
                                                    </div>
                                                    <div className="text-[10px] text-gray-600 dark:text-gray-400 mb-1">设备码:</div>
                                                    <div className="flex items-center gap-2">
                                                        <input
                                                            readOnly
                                                            value={copilotDeviceCode.user_code}
                                                            className="input input-xs input-bordered flex-1 font-mono text-center text-lg font-bold tracking-wider"
                                                            style={{ fontFamily: 'monospace', letterSpacing: '0.2em' }}
                                                        />
                                                        <button
                                                            onClick={() => {
                                                                navigator.clipboard.writeText(copilotDeviceCode.user_code);
                                        ...
                                        <div className="space-y-1">
                                            <label className="text-[11px] font-medium text-gray-500 dark:text-gray-400">
                                                {t('proxy.config.copilot.dispatch_mode')}
                                            </label>
                                            <select
                                                className="select select-sm select-bordered w-full text-xs"
                                                value={appConfig.proxy.copilot?.dispatch_mode || 'off'}
                                                onChange={(e) => updateCopilotConfig({ dispatch_mode: e.target.value as any })}
                                            >
                                                <option value="off">{t('proxy.config.zai.modes.off')}</option>
                                                <option value="exclusive">{t('proxy.config.zai.modes.exclusive')}</option>
                                                <option value="pooled">{t('proxy.config.zai.modes.pooled')}</option>
                                    ...
                                    <div className="space-y-1">
                                        <label className="text-[11px] font-medium text-gray-500 dark:text-gray-400">
                                            {t('proxy.config.copilot.default_model')}
                                        </label>
                                        <input
                                            type="text"
                                            value={appConfig.proxy.copilot?.default_model || 'claude-opus-4.5'}
                                            onChange={(e) => updateCopilotConfig({ default_model: e.target.value })}
                                            className="input input-sm input-bordered w-full font-mono text-xs"
                                            placeholder="claude-opus-4.5"
                                        />
                                        <div className="text-[10px] text-gray-400 flex items-center gap-2">
                                            <span>{t('proxy.config.copilot.supported_models')}:</span>
                                            <span className="font-mono">claude-opus-4.5</span>
                                            <span className="font-mono">claude-sonnet-4.5</span>
                                            <span className="font-mono">gpt-4.1</span>
                                            <span className="font-mono">o1-mini</span>
                                        </div>
                                    </div>
                                </div>
                            </CollapsibleCard>
